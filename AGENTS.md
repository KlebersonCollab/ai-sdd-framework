...
2. sdd-planner  (Feature planning: plan.md, spec.md, tasks.md)
3. sdd-executor (Atomic TDD implementation & sensor evidence)
4. sdd-review   (Sensor audit, AC verification & report)
```
...
* **Responsibilities**: Define scope (`plan.md`), BDD acceptance criteria with sensors (`spec.md`), and atomic tasks (`tasks.md`) under `.specs/features/<feature-id>/`. Create numbered ADRs in `.specs/project/ADRs/` for structural choices.
...
* **Responsibilities**: Follow `tasks.md` sequentially, apply TDD cycle, run sensors (lint, test, build), record commit/test evidence.
...
* **Forbidden**: Modifying `plan.md` or `spec.md` directly; marking tasks complete without sensor evidence.
...
* **Responsibilities**: Run sensors (Linter, Test Suite, Build), audit each AC against concrete code lines (`file:///...`), generate formal Verification Report with Verdict (`APPROVED` / `REQUESTS CHANGES`).
* **Reference Standards**: Consult [BDD Guide](.agents/skills/sdd-review/references/bdd-guide.md).
* **Forbidden**: Approving work without sensor evidence or skipping edge-case checks.
